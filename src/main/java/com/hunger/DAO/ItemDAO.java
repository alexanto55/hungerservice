...
public class ItemDAO {
    ...
    public GetQueryResultsResponse fetchMenuListDAO(String category) throws IOException, GeneralSecurityException{
        ...
        String itemSQL = " SELECT item_id, item, item_desc, unit_price " +
                "FROM ProjectData.item I " +
                "INNER JOIN ProjectData.Item_Category IC ON I.item_catgry = IC.Category_Code" +
        ...
        /** execute the query **/
        QueryResponse query = bq.jobs().query(ProjectID, new QueryRequest().setQuery(itemSQL)).execute();
